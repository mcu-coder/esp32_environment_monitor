...
import com.example.ESP32SmartMonitor.R
import com.example.ESP32SmartMonitor.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun eventManager() {
        ...
        binding.powerButton.setOnClickListener {
            binding.powerButton.isSelected = !binding.powerButton.isSelected
            sendMessage(2, if (binding.powerButton.isSelected) "0" else "1")
        }
        ...
        binding.TempSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(p0: SeekBar?, p1: Int, p2: Boolean) {
                if (Common.mqttHelper?.connected == true) {
                    binding.TempSeekText.text = p1.toString()
                } else {
                    MToast.mToast(this@MainActivity, "请先建立连接")
                }
            }
            ...
            when (cmd) {
                ...
                1 -> {
                    str = Gson().toJson(
                        Send(
                            cmd, DataDTO(
                                humi_v = binding.waterSeekBar.progress,
                                temp_v = binding.TempSeekBar.progress
